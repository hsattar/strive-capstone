...
export default function SingleWebsite({ website, handleDeleteWebsite }: IProps) {
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault()
        setUserError(false)
        if (confirmDeleteText === website.name) {
            handleDeleteWebsite(website.name, website._id)
        } else {
            setUserError(true)
        }
    }
    ...
    return (
        ...
        <div className="flex flex-col py-6">
            ...
            <a href={`/ws-preview/${website.name}/home`} target="_blank" className="text-xl ml-1">{website.name}</a>
                <div className="flex">
                    ...
                    <button onClick={() => setShowDeleteModal(true)} className="mr-1 select-none">
        ...
        { showDeleteModal && (
        <div onClick={() => setShowDeleteModal(true)} className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
            <div onClick={e => e.stopPropagation()} className="relative top-10 mx-auto p-5 border w-[50%] shadow-lg rounded-md bg-white">
                <form onSubmit={handleSubmit} autoComplete="off" noValidate className="mt-3">
                    ...
                    <button type="submit" onClick={e => e.stopPropagation()} className="bg-red-500 hover:bg-red-600 py-1 px-5 mr-3 rounded-md text-white">Delete</button>
                    </div>
                </form>
            </div>
        </div>
